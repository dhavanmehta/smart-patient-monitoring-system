(function ($) {
	...
	var _data = [
		0, 0, 0,  0.0000050048828125, 0.0000137939453125, 0.000049560546875, 0.00008740234375, 0.00015966796875,
...
<<<<<<< HEAD
						...
						interval = 60 * 1000 / (_data.length * heartRate);
...
>>>>>>> 5ae82bd11aff04bbe5aac7ea55081d8f3edfa764
                    	$("#temp").text(res['tempreture']);
                     	$("#heartrate").text(res['pulse_rate']);
 			 			ecg = res['ecg'];			                            
 			 			var heartRate = ecg; // bpm
 			 			i = 1;
<<<<<<< HEAD

 			 			/*if(res['tempreture']>35 || res['tempreture']<30)
=======
 			 			if(res['tempreture']>35 || res['tempreture']<30)
>>>>>>> 5ae82bd11aff04bbe5aac7ea55081d8f3edfa764
 			 			{
 			 				if (i == 1) {
	 					   $("tempr").css("background-color", "yellow");

 							} else {
							$("#tempr").backgroundColor = '#ffff00';
  							}
 							if(i == 1) i = 0; else i = 1;
<<<<<<< HEAD
 			 			}*/
								interval = 60 * 1000 / (_data.length * heartRate);
								var ecgval = setInterval(function () {
									$('.jke-ecgChart').ecgChart('addDataPoint', getDataPoint());
